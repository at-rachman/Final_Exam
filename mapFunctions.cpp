...
char getMapSquare(const int x, const int y)
{
  ...
  {
    result = MAP[y][x]; // #10 Atmanjaya Updated 5/17
  }
  return result;
}
...
bool setMapSquare(const int x, const int y, const char currentValue, const char newValue)
{
  ...
  {
    MAP[y][x] = newValue; // #11 Atmanjaya Updated 5/17
    result = true;
  }
...
{
  char mapSquare = getMapSquare(x, y);
  return (mapSquare == MAP_SQUARE_EMPTY || mapSquare == MAP_SQUARE_CHASM || mapSquare == MAP_SQUARE_KEY || mapSquare == MAP_SQUARE_PEBBLE || mapSquare == MAP_SQUARE_PEBBLES || mapSquare == MAP_SQUARE_PLANK || mapSquare == MAP_SQUARE_PLANK_SET || mapSquare == MAP_SQUARE_ROPE || mapSquare == MAP_SQUARE_ROPE_TIED || mapSquare == MAP_SQUARE_SLINGSHOT); /* #12 return true */
}
...
bool saveGame(string fileName, const int playerX, const int playerY, const char lookingDirection, const int werewoflX, const int werewolfY, const char inventoryItems[], const int inventoryValues[], const int inventoryLength, const int werewolfHealth, const int werewolfStunCount)
{
  ...
  if (!file.fail())
  {
    ...
    for (int i = 0; i < MAP_HEIGHT; i++)
    {
      ...
      {
        file << MAP[i][j];/* MISSING CODE - 1 line */
      }
      file << endl;
    }
